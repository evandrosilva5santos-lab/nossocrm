# StartCRM MCP Server — 100% Coverage

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Migrate the MCP server to `mcp-handler` (Vercel) and expand from 29 tools to full CRM coverage (~65 tools), enabling AI agents to operate StartCRM as first-class users.
...
**NOTE:** The API key should be created via the StartCRM admin UI. Do NOT commit the actual key — add `.mcp.json` to `.gitignore` or use environment variable interpolation if supported.
