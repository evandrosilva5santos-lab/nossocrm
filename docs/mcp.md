...
StartCRM exposes a **remote MCP (Model Context Protocol) server** that gives AI assistants and developer tools direct, authenticated access to your CRM data. Clients can search deals, manage contacts, send messages, review AI-suggested stage advances (HITL), and more — all via standardized MCP tool calls.
...
Every request must carry a valid StartCRM API key. Two header formats are accepted:
...
API keys are created in **StartCRM → Settings → API Keys**. Each key is scoped to a single organization; all tool calls are automatically filtered to that organization's data.
